...
<body class="easyui-layout">
	...
	<div title="系统菜单" data-options="region:'west'" style="width: 200px">
		...
		<div class="easyui-accordion" data-options="fit:true">
			...
			<div title="面板二">
				<!-- 展示树形菜单 ：使用标准json数据构造 ，不建议使用这种方式，因为描述数据使用的嵌套的方式，层级过多的话很不合适-->
				<ul id="ztree1" class="ztree"></ul>
				<script type="text/javascript">
					$(function() {
						//当页面加载完成后，动态创建ztree菜单
						var setting = {};//设置ztree相关的属性 ,使用默认值
						//构造json数据，在数组当中进行添加
						var zNodes = [ {
							name : '系统管理'
						},//每个json对象对应一个节点数据
						...
						//创建ztree：第一个参数是id,第二个参数是zTree相关属性，第三个参数是zTree节点
						$.fn.zTree.init($("#ztree1"), setting, zNodes);
					});
				</script>
			</div>
			<div title="面板三">
				<!-- 展示树形菜单 ：使用简单json数据构造，结构比较固定-->
				...
				<script type="text/javascript">
					$(function() {
						var setting3 = {
							...
							//回调函数
							callback : {//绑定事件,第一个参数代表：事件，第二个参数代表：树id,第三个参数代表：树节点
								onClick : function(a, b, treeNode) {
									//alert(treeNode.page);
									var page = treeNode.page;
									if (page != undefined) {//判断当前点击的节点是否有page属性，打开选项卡
										//判断当前选项卡是否已经打开,已经打开就不需add，重新切换一下
